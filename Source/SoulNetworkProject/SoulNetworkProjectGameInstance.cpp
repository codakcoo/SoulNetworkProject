...
void USoulNetworkProjectGameInstance::StartSession()
{
	if(SessionInterface)
	{
		...
		SessionInterface->StartSession(SESSION_NAME);	//Session을 시작하면 이서버를 리스트에 노출을 안시킴//
...
void USoulNetworkProjectGameInstance::Join(uint32 Index)
{
	...
	//if(MainMenuWidget) MainMenuWidget->Teardown();	//MenuWidget 전용//
...
void USoulNetworkProjectGameInstance::RefreshServerList_Implementation()
{
	...
	if (SessionSearch)
	{
		//SessionSearch->bIsLanQuery = true;	//로컬 매칭여부
		SessionSearch->MaxSearchResults = 1000;	//서버 탐색범위
		SessionSearch->QuerySettings.Set(SEARCH_PRESENCE, true, EOnlineComparisonOp::Equals);	//인터넷서버 필터같음 개념
		SessionInterface->FindSessions(0, SessionSearch.ToSharedRef());							//서버 찾기
...
void USoulNetworkProjectGameInstance::SearchServerList(FName SessionName)
{
	...
	if (SessionSearch)
	{
		if (IOnlineSubsystem::Get()->GetSubsystemName() == "NULL") SessionSearch->bIsLanQuery = true;	//로컬 매칭
...
void USoulNetworkProjectGameInstance::OnFindSessionComplete(bool Success)
{
	...
	if (Success && SessionSearch && MainMenuWidget)
	{
		...
		//TArray<FTestServerData> ServerNames;	//MenuWidget 전용//
		//테스트용 서버//
		//FServerData Data2;
		...
		for (auto& SearchResult : SessionSearch->SearchResults)
		{
			...
			//FTestServerData Data;	//MenuWidget 전용//
			FServerData Data;
		...
		//if(MainMenuWidget) MainMenuWidget->SetServerList(ServerNames);	//MenuWidget 전용//
...
void USoulNetworkProjectGameInstance::CreateSession()
{
	if (SessionInterface)
	{
		...
		//섹션 세팅을 설정 안할경우 섹션은 0개이다.//
		if (IOnlineSubsystem::Get()->GetSubsystemName() == "NULL") SessionSettings.bIsLANMatch = true;	//로컬 매칭
		else SessionSettings.bIsLANMatch = false;
		SessionSettings.NumPublicConnections = 4;	//플레이어 수
		SessionSettings.bShouldAdvertise = true;	//광고
