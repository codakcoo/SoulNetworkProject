...
void ULowImpactDamageType::ApplyDamageEffect(AActor* InCauser, AActor* InApply, FVector InDirection, float DamageValue)
{
		...
		if(Causer->GetCombatComponent() && Applier->GetStateComponent())
		{
			...
			if ((CurrentDeltaYaw >= -180.f && CurrentDeltaYaw < -130.f) || (CurrentDeltaYaw <= 180.f && CurrentDeltaYaw > 130.f))
			{
				if (Applier->GetCombatComponent()->GetGuardup())
				{
					...
					Applier->GetStateComponent()->AddStamina(-10.0f);
...
void ULowImpactDamageType::BasicDamageEvent(ACharacterBase* InCauser, ACharacterBase* InApply, FVector InDirection, float DamageValue)
{
