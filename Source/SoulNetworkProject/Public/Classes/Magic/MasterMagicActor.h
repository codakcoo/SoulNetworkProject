...
public:
	//기본적인 추상화 함수들// *자식 객체에 오버라이딩 해야함*

	virtual void BeginData(class ACharacterBase* InOwner, class USkeletalMeshComponent* CastMesh) PURE_VIRTUAL(AMasterMagicActor, );	//Begin시 호출 함수//

	virtual void LoopData() PURE_VIRTUAL(AMasterMagicActor, );	//Loop시 호출 함수//

	virtual void MultipleData(class ACharacterBase* InOwner) PURE_VIRTUAL(AMasterMagicActor, );	//Multiple 호출 함수// *상황에 따라 재정의 하거나 안해도 됨*

	virtual void EndData() PURE_VIRTUAL(AMasterMagicActor, );	//End시 호출 함수//

protected:
	//보통 Niagara, 위치, 회전하거나 Detach할때만 호출해서 사용함.//
